...
def add_densepose_sds_head_config(cfg: CN):
    _C = cfg
    _C.MODEL.DENSEPOSE_SDS_ON = False
    _C.MODEL.ROI_DENSEPOSE_SDS_HEAD = CN()
    _C.MODEL.ROI_DENSEPOSE_SDS_HEAD.CFGDIR = ""
    _C.MODEL.ROI_DENSEPOSE_SDS_HEAD.USE_IMAGE_AS_COND = False
    _C.MODEL.ROI_DENSEPOSE_SDS_HEAD.SDS_LOSS_WEIGHT = 1.0

def add_densepose_solver_config(cfg: CN):
    _C = cfg
    _C.SOLVER.OPTIMIZER_TYPE = "SGD"
    _C.SOLVER.BETAS = [0.9,0.99]
    _C.SOLVER.EPS = 1.e-15

def add_densepose_config(cfg: CN):
    add_densepose_head_config(cfg)
    add_densepose_sds_head_config(cfg)
    add_hrnet_config(cfg)
    add_bootstrap_config(cfg)
    add_dataset_category_config(cfg)
    add_evaluation_config(cfg)
    add_densepose_solver_config(cfg)
